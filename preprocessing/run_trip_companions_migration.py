...
def main():
    print("Connecting to Supabase PostgreSQL database to run trip companions migration...")
    try:
        conn = psycopg2.connect(conn_str)
        conn.autocommit = True
        cursor = conn.cursor()
        
        sql_file_path = os.path.join(os.path.dirname(__file__), "../sql/sync_trip_companions_trigger.sql")
        ...
        cursor.execute(sql)
        print("✅ Trip companions migration and trigger successfully installed and executed!")
